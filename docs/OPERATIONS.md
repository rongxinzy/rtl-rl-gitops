...
Argo CD 3.5.2 resolves remote refs with go-git; this does not run the git executable wrapper. The repo-server pins github.com to the verified reachable GitHub endpoint 20.201.28.151, with ARGOCD_GIT_ATTEMPTS_COUNT=3 and ARGOCD_GIT_REQUEST_TIMEOUT=10s. The cluster DNS endpoint 20.205.243.166 timed out repeatedly, while the selected endpoint returned HTTP 200 in three consecutive smart-HTTP probes (about 1.2 seconds). Do not restore multiple conflicting hostAliases or untested DNS routing. Recheck the endpoint against GitHub official metadata and live TLS-verified smart-HTTP probes when maintaining this pin. This bounds nominal ref-request time to 30 seconds and retries transient failures. The existing bounded git wrapper remains for fetch/clone. TLS verification remains enabled. See upstream util/git/client.go and common/common.go at v3.5.2.
