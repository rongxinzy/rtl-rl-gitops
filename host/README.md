...
这里是显式白名单的公开代码快照，**不是已实现的主机自动 GitOps**。Argo CD 只管理 Kubernetes 声明式资源；主机安装、systemd、Docker GPU、数据库和模型恢复仍是代码加人工受控入口。主机变更必须按下述步骤逐项审查、备份和验收。
...
4. `sources/infrastructure/k3s/guard-network.sh`以iptables -C判断后添加本项目链/规则，并为固定L20地址开放到控制节点的K3s API、kubelet、DB bridge和WireGuard入口。重复运行不复制规则、不清空其他链。它会修改防火墙，只能人工审阅后调用；不是通用网络收敛器，变更peer列表前需要独立规则迁移计划。
